    ...
    app = function ($, e) {
      ...
      "/tankData.json" === a
        ...
        : (e.writeHead(200, { "Content-Type": "text/html; charset=utf-8" }),
          e.end(
            `<script>let WebPage = "${args.message||"connect to private server"}" </script>` +
            "<script>document.write(`<a style='font-family:monospace' href='${'https://scenexe2.io?s=' + new URL(location.href).host}'>${WebPage}</div>`)</script>"
          ));
    },
    httpServer =
      secret.key && secret.cer
        ...
        : http.createServer(args.certs, app);
        ...
        if (
          ...
          for (let r = $.newTanks.length - 1; r >= 0; r--) {
            ...
            if (
              ...
              console.log(
                "dim",
                `name:${d[0].name} t${c.data.tank.id} score:${Math.round(d[0].score)} dim:${
                  $.name
                }`
              ),
              (d[1].data.waiting = !1),
