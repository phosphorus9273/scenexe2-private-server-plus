...
!function() {
  const dim = dimension.create({
    mapSize: 14000,                        // mapsize
    name: 'crossroadsLobby',                         // internal name of dim
    type: 'ffa',                       // leave this
    freeJoin: false,                       // whether tank can join from server select
    allowScale: true,                     // allow use of /mapsize command
    removeFallens: true,                  // remove tanks that go fallen
    displayName: 'crossroads',    // name of dim shown on server select
    displayRadiant: 0,                  // make color radiant on server select
    displayColor: -2,
    darkness: 1, // color on server select
    walls: ['crossroads'
     ],
    gates: [],
    background: {                         // background color
    ...
    spawnPlayer: function(team, tank) {
      ...
      setTimeout(function() {
        if(tank.ws.sendPacket) {          // send a notification
          tank.ws.sendPacket('announcement', 'Welcome to sandbox!')
        }
      })
